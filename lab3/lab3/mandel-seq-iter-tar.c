...
{
      int equal1, equal2;
      tareador_disable_object(&equal1);
      tareador_disable_object(&equal2);

      for (int y = 0; y < ROWS; y += TILE) {
	    for (int x = 0; x < COLS; x += TILE) {
		  tareador_disable_object(&x);
		  tareador_disable_object(&y);
		  tareador_disable_object(&M);
#ifdef og
		  tareador_start_task("mandelbrot");
#else
		  tareador_start_task("horizontal_border");
#endif
		  equal1 = 1;
...
#ifndef og
		  tareador_end_task("horizontal_border");
		  tareador_start_task("vertical_border");
#endif
		  equal2 = 1;
...
#ifndef og
		  tareador_end_task("vertical_border");
#endif
		  ...
		  if (equal1 || equal2) {
			...
			for (int py = y; py < y + TILE; py++) {
#ifndef og
				tareador_start_task("compute_py");
#endif
			      for (int px = x; px < x + TILE; px++) {
