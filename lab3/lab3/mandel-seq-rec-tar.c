...
#ifdef LEAF
      tareador_end_task("base_fill");
#endif
      } else {
	    if (NCols <= TILE) {
		  // computation of tile: base case
#ifdef LEAF
      tareador_start_task("base_compute");
#endif
		  for (int py = y; py < y + NRows; py++)
			for (int px = x; px < x + NCols; px++) {
